    ...
    if ok {
        if let (Some(exp_mina), Some(mina_usd), Some(zec_usd), Some(decimals)) = (
            ...
            if mina_usd_val > 0.0 && zec_usd_val > 0.0 {
                ...
                let source = match state.config.escrow_address_type {
                    crate::config::AddressType::Shielded => crate::zcashd::SpendSource::Shielded,
                    crate::config::AddressType::Transparent => crate::zcashd::SpendSource::Transparent,
                };
                let balance = state
                    .wallet
                    .rpc()
                    .get_balance(source, state.wallet.address())
                    .await
                    .unwrap_or(0.0);
                // Allow 10% slippage
...
async fn funding_shielded(
    ...
    let has_funds = state
        ...
        .has_shielded_funding(
            escrow_addr,
            &req.api_key, // memo must match API key
...
async fn funding_transparent(
    ...
    let balance = state
        ...
        .rpc()
        .get_balance(
            crate::zcashd::SpendSource::Transparent,
            state.wallet.address(),
        )
        .await?;
...
async fn send_back(
    ...
    let txid = state
        .wallet
        .sweep_full_balance(&binding.origin_address, &fee_policy)
        .await?;
...
async fn send_target(
    ...
    let txid = state
        .wallet
        .sweep_full_balance(&req.target_address, &fee_policy)
        .await?;

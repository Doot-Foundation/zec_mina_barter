...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        let status = match self {
            ...
            AppError::Wallet(_) => StatusCode::INTERNAL_SERVER_ERROR,
            AppError::Io(_) | AppError::Http(_) | AppError::Serde(_) => {
...
mod tests {
    ...
    fn test_error_display_simple_variants() {
        ...
        assert_eq!(AppError::TransitMismatch.to_string(), "in-transit gate mismatch");
        assert_eq!(AppError::Crypto.to_string(), "crypto error");
        assert_eq!(AppError::AlreadyBound.to_string(), "already bound");
        assert_eq!(AppError::InsufficientFunds.to_string(), "insufficient funds");
        assert_eq!(AppError::FundingNotFound.to_string(), "funding not found on-chain");
    }

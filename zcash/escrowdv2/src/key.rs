use crate::config::{AddressType, Config, Network};
...
use zcash_protocol::consensus::{Network as ConsensusNetwork, Parameters};
...
impl KeyManager {
    pub fn init(config: &Config) -> Result<Self, AppError> {
        ...
        let (ua, ufvk, transparent_addr) = derive_addresses(&seed, &consensus)?;
    ...
    pub fn escrow_address(&self, addr_type: AddressType) -> &str {
        match addr_type {
            AddressType::Shielded => &self.ua,
            AddressType::Transparent => &self.transparent_addr,
        }
    }
    ...
    pub fn unified_spending_key(&self) -> Result<UnifiedSpendingKey, AppError> {
        let account = AccountId::try_from(0u32).map_err(|_| AppError::Crypto)?;
        UnifiedSpendingKey::from_seed(&self.consensus, &self.seed, account)
            .map_err(|_| AppError::Crypto)
    }

    pub fn seed_secret(&self) -> SecretVec<u8> {
        SecretVec::new(self.seed.to_vec())
    }
...
fn derive_addresses(
    ...
    let net_type = params.network_type();
    let t_address = ua
        .transparent()
        .ok_or(AppError::Crypto)?
        .to_zcash_address(net_type)
        .encode();
    Ok((ua_encoded, ufvk_encoded, t_address))
}
...
mod tests {
    ...
    fn test_key_manager_init_loads_existing_keys() {
        ...
        assert_eq!(
            ufvk1, ufvk2,
            "UFVK should be identical when loading existing keys"
        );
    }
    ...
    fn test_seal_and_unseal_seed() {
        ...
        let original_seed = vec![
            ...
            0xc3, 0xd2, 0xe1, 0xf0, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa,
            0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x00, 0x00, 0xff, 0xee, 0xdd, 0xcc, 0xbb, 0xaa, 0x99,
        ...
        assert_eq!(
            original_seed, unsealed_seed,
            "Unsealed seed should match original"
        );
    }
    ...
    // TODO: Fix these tests - derive_ua_and_ufvk_from_seed function doesn't exist
    // Need to refactor to use KeyManager methods instead
    /*
    #[test]
    fn test_derive_ua_and_ufvk_deterministic() {
        ...
        // Verify format
        assert!(
            ua1.starts_with("utest"),
            "UA should start with utest for testnet"
        );
    ...
    */
    ...
    fn test_cleanup_after_send() {
        ...
        assert!(
            sealed_path.exists(),
            "Sealed key should exist before cleanup"
        );

        km.cleanup_after_send().unwrap();
        assert!(
            !sealed_path.exists(),
            "Sealed key should be removed after cleanup"
        );
    }

...
/**
 * Scenario 1 - MINA Sell Initialization: Lock Trade
 *
...
async function main() {
  logHeader('Scenario 1: MINA Sell - Step 3: Lock Trade');
  ...
  try {
    ...
    const currentState = loadTradeState('msi');
    await cleanupFailedTrade('msi', currentState);
    process.exit(1);
  }
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      ...
      if (status.in_transit) {
        ...
        updateTradeState('msi', { escrowdInTransit: true });
        break;
      }
  ...
  const finalState = loadTradeState('msi');
  ...
  console.log('  Run: node build/src/scripts/mina_sell_initialization/4_msi_claim.js');

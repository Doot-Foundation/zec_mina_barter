...
/**
 * Scenario 1 - MINA Sell Initialization: Settlement
 *
...
async function main() {
  logHeader('Scenario 1: MINA Sell - Step 5: Settlement');
  ...
  // console.log(
  //   `  ✅ Settlement transaction: ${sentTx.hash.slice(
  //     0,
  //     10
  ...
  console.log(
    '  Settlement is the process of committing all pending offchain state'
  );
  console.log(
    '  changes to the on-chain Merkle root commitment. After settlement:'
  );
...
main()
  ...
  .catch((error) => {
    ...
    console.error(
      '  - Verify there are pending offchain state actions to settle'
    );
    console.error(
      '  - Ensure you have sufficient memory (proof generation is intensive)'
    );
